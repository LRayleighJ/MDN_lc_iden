...
storedir = "/scratch/zerui603/KMT_simu_lowratio/qseries/10to15longtest/"
    ...
    for index_slc in range(num_bthlc):
        ...
        for i_4 in range(50):
            try:
                ...
                lc_fit_minimize_fortest = mag_cal(times,*args_minimize_fortest)
                
                chi_s_minimize = chi_square(lc_fit_minimize,lc_noi,sig)
                chi_s_minimize_fortest = chi_square(lc_fit_minimize_fortest,lc_noi,sig)
        ...
        data_array=np.array([args_data_test,list(times),list(d_times),list(lc_noi),list(sig),list(magnitude_tran(lc,basis_m)),list(args_minimize),list(args_minimize_fortest),list(lc_fit_minimize),list(lc_fit_minimize_fortest)],dtype=object)
