...
import netmodule.netMDN_te as lcnet
import datamodule.datahist as dhist

# initialize preload netparams
reload = 1
preload_Netmodel = "/home/zerui603/MDN_lc/testnet/netparams/net_params200sk6args.pkl"

# initialize the storage path
rootval = "/scratch/zerui603/KMTsimu_te/val/"
...
for index_sample in range(test_size):
    ...
    # Silly code......
    pi1,pi2,pi3,pi4,pi5,pi6,mu1,mu2,mu3,mu4,mu5,mu6,sigma1,sigma2,sigma3,sigma4,sigma5,sigma6 = network(inputs)
    pi1 = pi1.detach().cpu().numpy()[0]
    pi2 = pi2.detach().cpu().numpy()[0]
    pi3 = pi3.detach().cpu().numpy()[0]
    pi4 = pi4.detach().cpu().numpy()[0]
    pi5 = pi5.detach().cpu().numpy()[0]
    pi6 = pi6.detach().cpu().numpy()[0]
    pis = np.exp(np.array([pi1,pi2,pi3,pi4,pi5,pi6]))
    mu1 = mu1.detach().cpu().numpy()[0]
    mu2 = mu2.detach().cpu().numpy()[0]
    mu3 = mu3.detach().cpu().numpy()[0]
    mu4 = mu4.detach().cpu().numpy()[0]
    mu5 = mu5.detach().cpu().numpy()[0]
    mu6 = mu6.detach().cpu().numpy()[0]
    ...
    sigma5 = sigma5.detach().cpu().numpy()[0]
    sigma6 = sigma6.detach().cpu().numpy()[0]
    sigmas = np.array([sigma1,sigma2,sigma3,sigma4,sigma5,sigma6])
    ...
    t0_domain = np.linspace(t0-range_t0,t0+range_t0,200)
    te_domain = np.linspace(te-range_te,te+range_te,200)

    args_domains = np.array([lgq_domain,lgs_domain,ux_domain,uy_domain,t0_domain,te_domain])
    axislabels = ["lg q","lg s","ux","uy","t0","te"]

    # Plot density figure
    mat_id = (np.array(range(5*5))+1).reshape((5,5)).T
    plot_base = 320

    plt.figure(figsize=(30,30)) 
    for index_i in range(5):
        for index_j in range(index_i+1,6):
            ...
            plt.subplot(5,5,mat_id[index_i][index_j-1])
            ...
            # plt.axis("equal")
            plt.grid()
    plt.suptitle(r"$\lg q$=%.3f,$\lg s=$%.3f,$u_0=$%.3f,$\alpha=$%.1f,$t_0$=%.3f,$t_E$=%.3f,ID=%d"%(lg_q,lg_s,labels[0],labels[4],labels[7],labels[5],index_list[index_sample]),fontsize=40)
    ...
    plt.scatter(time_ref,lc_ref,s=20,c="r",marker="x",zorder=1)
    plt.title(r"$\lg q$=%.3f,$\lg s=$%.3f,$u_0=$%.3f,$\alpha=$%.1f,$t_0$=%.3f,$t_E$=%.3f,ID=%d"%(lg_q,lg_s,labels[0],labels[4],labels[7],labels[5],index_list[index_sample]))
    ...
    plt.axis("scaled")
    plt.grid()
    plt.suptitle(r"$\lg q$=%.3f,$\lg s=$%.3f,$u_0=$%.3f,$\alpha=$%.1f,$t_0$=%.3f,$t_E$=%.3f,ID=%d"%(lg_q,lg_s,labels[0],labels[4],labels[7],labels[5],index_list[index_sample]),fontsize=30)
