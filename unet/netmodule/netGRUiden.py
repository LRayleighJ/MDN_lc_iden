...
def default_loader_fortest(data_root, posi_lc):
    ## [u_0, rho, q, s, alpha, t_E, basis_m, t_0, chi^2, label]
    ## [times, dtimes, lc_noi, sigma, lc_nonoi, args_minimize, lc_fit_minimize, chi_array]

    # compare with unet, unet as follow
    ...
    lc_withnoi = np.array(datadir[4],dtype=np.float64)
    lc_singlemodel = np.array(datadir[7],dtype=np.float64)
    lc_err = np.array(datadir[5],dtype=np.float64)

    chi_array = (lc_withnoi-lc_singlemodel)/lc_err
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.rnnout1(x)
        x = self.rnnout2(x)
        print(x.shape,"after rnn")
        # x = x.view(-1,1,1000)
        x = self.pre(x)
        print(x.shape,"after pre")
        x = self.body(x)
        print(x.shape," 5")
        x = x.view(-1,32*512)
        # print(x.shape," 6")
