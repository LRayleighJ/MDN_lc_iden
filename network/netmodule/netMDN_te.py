...
def default_loader(data_root,posi_lc):
    # [u_0, rho, q, s, alpha, t_E, basis_m, t_0, t[0],t[-1]]
    ...
    t_range = np.abs(labels[-1]-labels[-2])
    ...
    t_0_label = (t_0-labels[-2])/t_range
    t_E_label = t_E/t_range*4
    

    label = np.array([-1.*float(q_label),float(s_label),float(ux_label),float(uy_label),float(t_0_label),float(t_E_label)]).astype(np.float64)
...
class Mydataset(Dataset):
    def __init__(self,transform=None,target_transform=None,n_lc=None,data_root=None,loader=default_loader):
        self.n_lc = n_lc
    ...
    def __getitem__(self, index):
        lc, label = self.loader(self.data_root,index)
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(5*256, 864)
        self.fc2 = nn.Linear(864,864)
        self.fc3 = nn.Linear(864,512)
        self.fc4 = nn.Linear(512,512)

        self.opepi1 = nn.Linear(512,12)
        self.opepi2 = nn.Linear(512,12)
        self.opepi3 = nn.Linear(512,12)
        self.opepi4 = nn.Linear(512,12)
        self.opepi5 = nn.Linear(512,12)
        self.opepi6 = nn.Linear(512,12)

        self.opemu1 = nn.Linear(512,12)
        self.opemu2 = nn.Linear(512,12)
        self.opemu3 = nn.Linear(512,12)
        self.opemu4 = nn.Linear(512,12)
        self.opemu5 = nn.Linear(512,12)
        self.opemu6 = nn.Linear(512,12)

        self.opesigma1 = nn.Linear(512,12)
        self.opesigma2 = nn.Linear(512,12)
        self.opesigma3 = nn.Linear(512,12)
        self.opesigma4 = nn.Linear(512,12)
        self.opesigma5 = nn.Linear(512,12)
        self.opesigma6 = nn.Linear(512,12)
    ...
    def forward(self, x):
        ...
        pi5 = self.lsfm(self.opepi5(x))
        pi6 = self.lsfm(self.opepi6(x))
        ...
        sigma5 = torch.exp(self.opesigma3(x))
        sigma6 = torch.exp(self.opesigma4(x))
        
        return pi1,pi2,pi3,pi4,pi5,pi6,mu1,mu2,mu3,mu4,mu5,mu6,sigma1,sigma2,sigma3,sigma4,sigma5,sigma6
'''
...
class Loss_fn(nn.Module):
    def __init__(self):
        super().__init__()
    def forward(self,y,pi1,pi2,pi3,pi4,pi5,pi6,mu1,mu2,mu3,mu4,mu5,mu6,sigma1,sigma2,sigma3,sigma4,sigma5,sigma6):
        mixture1 = torch.distributions.normal.Normal(mu1, sigma1)
        # print(y.shape)
        # print(y.t()[0].t().shape)
        ...
        mixture5 = torch.distributions.normal.Normal(mu5, sigma5)
        log_prob5 = mixture5.log_prob(y.t()[4].t().unsqueeze(-1))
        weighted_logprob5 = log_prob5 + pi5
        log_sum5 = torch.logsumexp(weighted_logprob5, dim=-1)
        
        mixture6 = torch.distributions.normal.Normal(mu6, sigma6)
        log_prob6 = mixture6.log_prob(y.t()[5].t().unsqueeze(-1))
        weighted_logprob6 = log_prob6 + pi6
        log_sum6 = torch.logsumexp(weighted_logprob6, dim=-1)

        return -log_sum1.mean()-log_sum2.mean()-log_sum3.mean()-log_sum4.mean()-log_sum5.mean()-log_sum6.mean()