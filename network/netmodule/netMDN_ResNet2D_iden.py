...
def default_loader(data_root,posi_lc,prenum=0):
    # [u_0, rho, q, s, alpha, t_E]
    # datadir = pd.DataFrame(np.load(dataroot+str(posi_lc)+".npy", allow_pickle=True))
    datadir = list(np.load(data_root+str(posi_lc+int(prenum))+".npy", allow_pickle=True))
    ...
    return np.array([lc_mag]), label


class Mydataset(Dataset):
    def __init__(self,transform=None,target_transform=None,n_lc=None,data_root=None,prenum=0,loader=default_loader):
        self.n_lc = n_lc
    ...
    def __getitem__(self, index):
        lc, label = self.loader(self.data_root,index,self.prenum)
