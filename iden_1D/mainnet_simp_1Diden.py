...
import netmodule.netSimpiden as lcnet
import datamodule.datamethod as dm

num_code = np.int(sys.argv[1])
forbidden_numlist = [0,2,3,4,5,6,7]
...
# reload
reload = 0
preload_Netmodel = "GRUresnet_iden_simp_"+name_group+".pkl"
...
def testnet(datadir=rootval,fullorparttest = fullorparttest):
    ...
    plt.legend()
    plt.xlabel("$\log_{10} |\Delta \chi^2|$")
    plt.savefig("histbs_"+name_group+"_simp.png")
    plt.close()
    ...
    plt.savefig("test_binary_distribution_bins_"+name_group+"_simp.png")
    plt.close()
    ...
    plt.ylabel("$m_0$")
    plt.title(name_group)
    plt.savefig("test_bb_distribution_bins_"+name_group+"_simp.png")
    plt.close()
    ...
    plt.ylabel("$m_0$")
    plt.title(name_group)
    plt.savefig("test_binary_rate_bins_"+name_group+"_simp.png")
    plt.close()
...
def training():
    ...
    for epoch in range(n_epochs):
        ...
        plt.savefig("loss_accuracy_GRU1d_"+name_group+"_simp.png")
        plt.close()
